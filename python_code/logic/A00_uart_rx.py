...
#
# 지금은 두 보드가 같은 양식이다. 둘 다 뒤 두 자리를 먼저 보낸다.
# 나중에 온 바이트가 앞 두 자리다. (0x35 0x89 -> 8935)
#
# 이 값은 펌웨어를 고칠 때마다 따라 움직인다. 순서를 의심할 일이 생기면
# 터미널의 수신 줄을 먼저 볼 것. 받은 바이트와 함께 '반대로 읽으면 ____'가
# 찍히므로, 실물 번호판과 같은 쪽을 고르면 된다. 바꾸는 것은 실행 중에
# /byteorder/entry 또는 /byteorder/exit 한 번이면 되고, 그 값을 오래 쓸
# 것이면 여기에도 적어 둘 것. (set_byte_order)
#
...
BYTE_ORDER = {
    'entry': 'low_first',       # 입차 Zybo (뒤 두 자리를 먼저 보낸다)
    'exit': 'low_first',        # 출차 Zybo (입차와 같은 양식)
}
