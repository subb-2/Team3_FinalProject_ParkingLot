...
#
# 두 보드 다 앞 두 자리부터 보낸다. 입차를 low_first로 두었던 것은 실측이
# 아니었다. 시연 번호판이 1111, 2222처럼 앞뒤 두 자리가 같은 것뿐이라
# 뒤집혀도 같은 값이 나와 여태 드러나지 않았을 뿐이다. 실물 번호판을
# 등록하고 나서야 보였다. (입차 8935 -> 3589로 들어옴)
#
# 뒤집히면 등록되지 않은 번호가 되어 조용히 일반 차량으로 처리된다.
# 8935는 전기차인데 3589로 읽혀 일반 자리를 배정받았다.
# 한쪽 펌웨어가 순서를 바꾸면 그 역할만 여기서 되돌릴 것.
BYTE_ORDER = {
    'entry': 'high_first',      # 입차 Zybo
