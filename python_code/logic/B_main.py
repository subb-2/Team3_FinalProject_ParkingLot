...
CONFIG = {
    ...
    # C_main / E_main_final과 같은 1280x720을 쓴다.
    #
    # 예전에는 여기만 640x480이었다. B_main은 마커를 안 쓰니 낮춰도 된다고 본
    # 것인데, 그 바람에 검출/추적을 확인한 화각과 실제로 운용하는 화각이 달랐다.
    # 640x480은 4:3, 1280x720은 16:9라 담기는 범위가 다르고, 카메라에 따라서는
    # 저해상도에서 센서를 잘라 쓰기도 한다. 여기서 잘 잡히던 차가 최종 실행에서
    # 다르게 보이면 원인을 찾기 어렵다.
    #
    # 해상도 근거는 B00_camera_input의 DEFAULT_WIDTH 주석 참고.
