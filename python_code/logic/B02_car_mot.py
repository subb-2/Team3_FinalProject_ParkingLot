...
CONFIG = {
    ...
    "SINGLE_ACTIVE": {
        ...
        #
        # 10초로 둔다. 1초는 자리 앞에서 잠깐 멈칫한 것과 세운 것을 가리지
        # 못했다. 차가 자리에 들어가 실제로 서는 데까지 걸리는 시간을 보고
        # 정한 값이다. 이 시간 동안 반경(ARRIVAL_RADIUS_CM)을 벗어나면
        # 처음부터 다시 센다.
        #
        # 안전장치(STUCK_RELEASE_SEC)보다 짧아야 한다. 그쪽이 먼저 걸리면
