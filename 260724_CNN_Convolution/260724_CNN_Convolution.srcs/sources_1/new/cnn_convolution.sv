...
module cnn_conv1 #(
  ...
  localparam logic [7:0] BIAS_PARAM[0:5] = '{8'hFF, 8'hFD, 8'hFA, 8'hEF, 8'h1B, 8'h01};
  ...
  assign bias = BIAS_PARAM[i_ch_cnt];
    ...
    always @ (posedge clk or posedge reset) begin
        if (reset) begin
            ...
            end else if (($signed(conv2_sum) >>> 8) > 21'sd255) begin
                o_pxl_data <= 8'd255;                        // Saturation
            end else begin
                o_pxl_data <= conv2_sum[15:8];               // Valid Range (0~255)
            end
        end
    end
